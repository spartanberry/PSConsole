...
function Invoke-PodeSignalEvent {
    ...
    if (!$PodeContext.Server.Signals.Connections.ContainsKey($Name) -or
        !$PodeContext.Server.Signals.Connections[$Name].Events.ContainsKey($strType) -or
        $PodeContext.Server.Signals.Connections[$Name].Events[$strType].Count -eq 0) {
        return
    }
    ...
    foreach ($evt in $PodeContext.Server.Signals.Connections[$Name].Events[$strType].Values) {
