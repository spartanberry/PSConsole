...
$sel = 'deviceName,userPrincipalName,operatingSystem,osVersion,isEncrypted,managedDeviceOwnerType,lastSyncDateTime'
Invoke-Graph "/deviceManagement/managedDevices?`$select=$sel" |
Where-Object { -not $UnencryptedOnly -or -not $_.isEncrypted } |
ForEach-Object {
    [PSCustomObject]@{
        Device      = $_.deviceName
...
} | Sort-Object Encrypted, Device   # unencrypted (False) first
